 ...
 * @since 1.11
 */
public class QPDualActiveSolverW extends QPOptimizer {
    /** Last warm state snapshot. */
    private WarmState lastWarmState;

    /** Container for warm-start data captured from the latest solve. */
    public static final class WarmState {
        private final RealVector x;
        private final RealVector u;
        private final List<Integer> active;
        private final QRUpdaterR qrUpdater;

        private WarmState(final RealVector x, final RealVector u, final List<Integer> active, final QRUpdaterR qrUpdater) {
            this.x = x;
            this.u = u;
            this.active = active;
            this.qrUpdater = qrUpdater;
        }
        ...
        public QRUpdaterR getQrUpdater() {
            return qrUpdater;
        }
    }

    public WarmState getLastWarmState() {
        return lastWarmState;
    }
    ...
    private double findPrimalStep(final RealVector z,
        ...
        // Compute the linear L2 norm of the search direction vector
        double norm2 = z.dotProduct(z);
        double zNorm = FastMath.sqrt(norm2);

        // Check if the search direction is computationally zero to avoid numerical noise
        if (zNorm <= Precision.EPSILON) {
            return Double.POSITIVE_INFINITY;
        }

        // Compute the projection of the direction vector onto the constraint normal
        double denom = ai.dotProduct(z);
        
        // Scale the zero-denominator threshold based on the magnitude of the constraint vector
        double aiNorm = ai.getNorm();
        double threshold = Precision.EPSILON * aiNorm;

        // Check if the direction vector is nearly orthogonal to the constraint plane
        if (FastMath.abs(denom) <= threshold) {
            return Double.POSITIVE_INFINITY;
        }

        // Calculate the step size along the search direction
        double alpha = -sv / denom;

        // Inequality constraints only block movement in the positive step direction
        if (!equality && alpha < 0) {
...
//            weights = computeConstraintWeights(C);
        ...
        return solveActiveSetLoop(x, u, active, G, g0, g, p, m, C, c0, weights, qrUpdater);
    }

    public LagrangeSolution optimizeWarmStart(final RealVector x,
                                              final RealVector u,
        ...
        this.maxIter = 40 * (G.getColumnDimension() + p + m);
        return solveActiveSetLoop(x, u, active, G, g0, g, p, m, C, c0, weights, qrUpdater);
    }

    private LagrangeSolution solveActiveSetLoop(final RealVector x,
                                                RealVector u,
                                                final List<Integer> active,
                                                final RealMatrix G,
                                                final RealVector g0,
                                                ...
                                                final RealVector weights,
                                                final QRUpdaterR qrUpdater) {
        final int n = G.getColumnDimension();
        final int mc = p + m;
        ...
        while (mc != 0 && iteration++ < maxIter) {
            ...
            for (int i = 0; i < active.size(); ++i) {
                if (active.get(i) >= p && u.getEntry(i) < 0.0) {
                    u.setEntry(i, 0.0);
                }
            }
...
//            }
            ...
            if (cvMax <= Precision.EPSILON && !refinementDone) {
                Pair<RealVector, RealVector> xy = recomputeXY(x, G, g0, C, c0, qrUpdater, active);
                x.combineToSelf(0.0, 1.0, xy.getFirst());
                u = xy.getSecond();
                kViolated = -1;
                refinementDone = true;
            ...
            if (!active.isEmpty()) {
                for (int j = 0; j < active.size(); j++) {
                    if (active.get(j) >= p && u.getEntry(j) < 0.0) {
                        qrUpdater.deleteConstraint(j);
                        active.remove(j);
                        xy = recomputeXY(x, G, g0, C, c0, qrUpdater, active);
                        x.combineToSelf(0.0, 1.0, xy.getFirst());
                        u = xy.getSecond();
                        j=FastMath.max(0, j-1);
                     }
            ...
            while (iteration++ < maxIter) {
                ...
                while (iteration++ < maxIter) {
                    ...
                    if (!Double.isFinite(t)) {
                        captureWarmState(x, u, active, qrUpdater);
    ...
    private void captureWarmState(final RealVector x,
                                  final RealVector u,
                                  final List<Integer> active,
                                  final QRUpdaterR qrUpdater) {
        this.lastWarmState = new WarmState(x.copy(), u.copy(), new ArrayList<>(active), qrUpdater);
    }
